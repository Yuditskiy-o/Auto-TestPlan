...
Системой сборки проекта будет Gradle (в качестве альтернативного варианта можно использовать Maven). Причины: быстрота сборки (пример cравнения с Maven [здесь](https://gradle.org/maven-vs-gradle/)), гибкость самой системы, производительность, UX (сравнивая build.gradle и pom.xml я отдаю предпочтение build.gradle за простоту и минимум кода).

- **Git**

Любому современному проекту будет необходима система контроля версий и Git — это как раз то, что нужно. В Git можно откатиться к предыдущему билду, можно вести параллельную разработку через систему веток, есть интеграция с IDEA.
...
Плагин, позволяющий генерировать тестовые данные — имя, пароль и т.п. Позволит решить проблему, когда необходимо где-то брать тестовые данные, чтобы не хардкодить их в методы.

## 3. Перечень необходимых разрешений/данных/доступов от банка

- Так как тестировать мы будем "живой" сайт, а не какую-то тестовую версию, то сначала необходимо получить разрешение от Альфа-банка на проведение подобного рода манипуляций.
- Доступы и какие-либо пароли будут не нужны, т.к. заявку может отправить абсолютно любой пользователь (в том числе и незарегистрированный ).
- Данные пользователей тоже не нужны, т.к. мы генерируем их с помощью Faker.

## 4. Перечень и описание возможных рисков при автоматизации
- Дополнительная нагрузка на сайт;
- Увеличение времени на тестирование (сначала проходимся руками по тест-кейсам, затем упаковываем это в код и т.д.) и, как следствие, стоимости;
- Ложные срабатывания из-за некорректно сгенерированных данных Faker'ом;
...
На мой взгляд для тестирования такой сравнительно небольшой функциональности понадобится один QA-инженер, владеющий навыки автоматизации и перечисленными инструментами.

## 6. Интервальная оценка с учётом рисков (в часах)

Если представить, что проект поручен специалисту, которые владеет всеми инструментами, то я бы предположил следующее:

- Уточнение требований, cогласования и получение разрешений — 2 часа;
- Написание авто-тестов + рефакторинг кода — 6 часов;
- Резерв времени на написание баг-репортов — 2 часа;
